...
class UserMenus extends BaseModel
{
 ...
 public function getAllMenus($usertypeid=null){
 	...
 	foreach($model as $m=> $k){
 		...
 		$model1=DB::table('menus')->select(['menuid','menu_nameen','menu_namenp','menu_parent_id'])->where('menu_parent_id', $k->menuid)->get();
 		foreach($model1 as $sm => $sk){
 			$data[$m]['parent']['childern'][$sm]= ['menuid'=> $sk->menuid,'menu_nameen'=>$sk->menu_nameen, 'menu_namenp' => $sk->menu_namenp,'menu_parent_id' => $sk->menu_parent_id];
 		$model1 = DB::table('menus')->select(['menuid','menu_nameen','menu_namenp','menu_parent_id'])->where('menu_parent_id', $sk->menuid)->get();
 		if($model1!='[]'){
