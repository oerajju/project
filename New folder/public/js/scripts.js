...
function getSelectedValue(id){
    var value =  $('#'+id+' option:selected').val() || $('#'+id).val();
    return '<input type="hidden" name="'+id+'[drb][]" value="'+(value==''?'-':value)+'">';
}
...
function readSignatureSeq(data){
    var value="";
    var splitString = data.split(',');
    for(var i in splitString){
        if(i==splitString.length-1){
        return value +=$('#'+splitString[i]).val();
        }        
        value += ($('#'+splitString[i]).val()).concat(',');
    }
}
